...
- Produce output that a reasonable reader would interpret differently from the original."""),
        ("human","""Below is a example of how to enhance text:
...
embedding_generator_runnable=RunnableParallel(
    {
        "enhanced_embedding": RunnableLambda(lambda x:generate_embedding(embedding_model,x['enhanced_text'])),
        "original_embedding": RunnableLambda(lambda x:generate_embedding(embedding_model,x['original_text'])),
...
similarity_score_runnable=RunnableParallel(
    {
        "enhanced_text": RunnableLambda(lambda x:x['enhanced_text']),
        "similarity_score": RunnableLambda(lambda x:cosine_similarity(x['enhanced_embedding'],x['original_embedding']))
    }
)

final_runnable=generator_runnable | embedding_generator_runnable | similarity_score_runnable
